...
项目使用 `.githooks/prepare-commit-msg` 自动为没有版本号的提交消息追加下一个 `v1.0.xx`。版本号会同时检查已有 `v1.0.*` 标签和历史提交消息，因此当前 `v1.0.57` 之后的下一个版本为 `v1.0.58`，后续提交会继续递增。已经包含版本号的提交不会重复追加。

新克隆仓库后执行以下命令启用共享 hook：
