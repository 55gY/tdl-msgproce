...
仓库提供共享 Git hook `.githooks/prepare-commit-msg`。新克隆仓库后执行：

```bash
./scripts/setup-git-hooks.sh
```

之后不带版本号的提交会自动追加下一个 `v1.0.xx`。版本号按已有 `v1.0.*` 标签及历史提交消息中的最大序号递增；已经包含版本号的提交不会重复追加。当前已有版本为 `v1.0.57`，下一次自动版本为 `v1.0.58`。
